...
LANGS = {
    ...
    'en': {
        ...
        'boiler_p': 'Behind every production line there are systems customers never see — but without them, nothing moves.',
        ...
        'cta_t': 'Interested?', 'cta_p': 'Write to us — we will get back to you with details about the role, shifts and fit for the energy team.', 'cta_b': 'Contact details will be added here',
    ...
    'ar': {
        ...
        'boiler_p': 'خلف كل خط إنتاج توجد أنظمة لا يراها الزبون — لكن من دونها لا يتحرك شيء.',
    ...
    'ru': {
        ...
        'boiler_p': 'За каждой производственной линией стоят системы, которые не видно на полке — но без них ничего не работает.',
        'comp_top': '02 · Компрессорная воздушных компрессоров',
        ...
        'cards': [('Техники / инженеры-практики с опытом','Люди, которые уже знают промышленные системы, энергию или обслуживание.'), ('В начале пути?','Молодые специалисты, которые хотят учиться настоящей профессии на месте.'), ('Опыт даже без диплома','Если есть реальный опыт с воздушными компрессорами, чиллерами или бойлерами — давай поговорим.')],
        'cta_t': 'Подходит тебе?', 'cta_p': 'Напишите нам — мы вернёмся с деталями по вакансии, сменам и соответствию команде энергии.', 'cta_b': 'Контакт будет добавлен здесь',
        'systems': ['Компрессоры','Чиллеры','Бойлеры','Энергия']
    },
    'hi': {
        'lang': 'hi', 'dir': 'ltr', 'label': 'हिन्दी',
        'brand': 'Strauss / Elite · Nof HaGalil',
        'title': 'असली सिस्टम। असली ज़िम्मेदारी।',
        'lead': 'हम maintenance practical engineers, energy technicians और air compressors, chillers और boilers में proven experience वाले लोगों की तलाश कर रहे हैं।',
        'section': 'यह सिर्फ़ एक और technical role नहीं है',
        ...
        'boiler_t': 'Factory का energy heart',
        'boiler_p': 'हर production line के पीछे ऐसी systems होती हैं जिन्हें customer नहीं देखता — लेकिन उनके बिना कुछ भी नहीं चलता।',
        'comp_top': '02 · Air compressor room',
        ...
        'cards': [('Experienced technicians / practical engineers','जो लोग industrial systems, energy या maintenance पहले से जानते हैं।'), ('Career की शुरुआत में?','Young practical engineers जो field से real profession सीखना चाहते हैं।'), ('Certificate के बिना भी experience','अगर air compressors, chillers या boilers में relevant hands-on experience है — बात करते हैं।')],
        'cta_t': 'Interested?', 'cta_p': 'हमें लिखें — हम role, shifts और energy team fit की details के साथ वापस संपर्क करेंगे।', 'cta_b': 'Contact details यहाँ जोड़ी जाएँगी',
        'systems': ['Air compressors','Chillers','Boilers','Energy']
    }
}
...
for code, data in LANGS.items():
    ...
    html = f'''<!doctype html><html lang="{data['lang']}" dir="{data['dir']}"><head>{HEAD}<meta property="og:title" content="{data['title']}"/><meta property="og:description" content="{data['lead']}"/><meta property="og:image" content="https://kopcaptz.github.io/strauss-elite-energy-recruitment/assets/processed/02-pipe-platform-sketch-a.jpg"/><title>{data['title']}</title><style>{CSS}</style></head><body><main class="wrap"><nav class="lang-switch" aria-label="Language switch">{links}</nav><section class="hero"><img src="../assets/processed/02-pipe-platform-sketch-a.jpg" alt=""><div class="hero-content"><div class="eyebrow">{data['brand']}</div><h1>{data['title']}</h1><p class="lead">{data['lead']}</p><div class="systems">{systems}</div></div></section><section class="section-title"><h2>{data['section']}</h2><p>{data['section_p']}</p></section><section class="story"><article class="photo"><img src="../assets/processed/01-boiler-room-sketch.jpg" alt=""><div class="photo-content"><div class="topline">01</div><div class="copybox"><h3>{data['boiler_t']}</h3><p>{data['boiler_p']}</p></div></div></article><article class="photo"><img src="../assets/processed/04-compressor-room-sketch.jpg" alt=""><div class="photo-content"><div class="topline">{data['comp_top']}</div><div class="copybox"><h3>{data['comp_t']}</h3><p>{data['comp_p']}</p></div></div></article></section><section class="section-title"><h2>{data['fit_t']}</h2><p>{data['fit_p']}</p></section><section class="fit">{cards}</section><section class="cta"><h2>{data['cta_t']}</h2><p>{data['cta_p']}</p><a class="button" href="#">{data['cta_b']}</a></section></main></body></html>'''
    (LANG_DIR / f'{code}.html').write_text(html)

(DOCS / 'multilingual.html').write_text('''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="refresh" content="0; url=lang/he.html"><title>Choose language</title><style>body{background:#07090c;color:#fff8ea;font-family:Arial,sans-serif;padding:24px}a{color:#ffb72d}</style></head><body><p>Choose language:</p><p><a href="lang/he.html">עברית</a> · <a href="lang/en.html">English</a> · <a href="lang/ar.html">العربية</a> · <a href="lang/ru.html">Русский</a> · <a href="lang/hi.html">हिन्दी</a></p></body></html>''')
